...
export default function Testimonials() {
  ...
  const slideTransform = isMobile
    ? `translateX(calc(-${current * 100}% - ${current * 50}px))`
    : `translateX(calc(-${current * (100 / 3)}% - ${current * (28 / 3)}px))`

  return (
    <section className="px-25 pb-35 max-lg:px-5 max-lg:pb-14">
      {/* Header */}
      <motion.div
      ...
      <motion.div
        className="bg-dark rounded-card pt-21 pb-17 max-lg:py-10"
        initial={{ opacity: 0 }}
      ...
      >
        {/* Slider */}
        <div className="overflow-hidden px-15 max-lg:px-5">
          <div
          ...
          >
            {testimonials.map((t, i) => (
              <motion.div
                ...
                style={{
                  width: isMobile ? '100%' : 'calc(55% - 18.67px)',
              ...
              >
                {/* Card */}
                <div className="border border-green rounded-card pt-12 px-13 pb-13 max-lg:p-8">
        ...
        {/* Navigation */}
        <div className="flex items-center justify-center gap-47.25 mt-31 max-lg:gap-10 max-lg:mt-8">
          <motion.button
            onClick={prev}
