...
function ArrowLink({ dark = false }: { dark?: boolean }) {
  return (
    <div className="flex items-center gap-3.75 mt-auto w-fit cursor-pointer group">
    ...
    <section className="pt-35 pb-25 px-25" id="services">
      ...
      <div className="grid grid-cols-2 gap-10 max-md:grid-cols-1 max-md:gap-6">
        {services.map((svc) => (
          <div
            key={svc.title}
            className={`${svc.bg} border border-[#191A23] rounded-card shadow-[0_5px_0_0_#191A23] p-12.5 min-h-77.5 flex gap-4 max-md:flex-col max-md:p-8`}
          >
            {/* Left: título + arrow */}
